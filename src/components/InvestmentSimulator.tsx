...
export default function InvestmentSimulator() {
  ...
  const handleCalculate = () => {
    ...
    if (isNaN(parsedBtcAmount) || isNaN(parsedMonthlyInvestment) || parsedBtcAmount < 0 || parsedMonthlyInvestment < 0) {
      setErrorMessage("入力値が不正です。適切な数値を入力してください。");
      return;
    }
    ...
    for (let i = 0; i < model.cagr.length; i++) {
      ...
      newResults.push({
        ...
        btcPriceRaw: btcPrice, // グラフ用のBTC価格（未フォーマット）
  ...
  return (
    <div className="p-4 bg-white text-gray-900">
      ...
      <div className="space-y-4">
        <div>
          <label className="block">保有BTC量</label>
          <input
            type="number"
            value={btcAmount}
            onChange={(e) => setBtcAmount(e.target.value)}
            className="w-full p-2 border border-gray-300 rounded"
            placeholder="例: 0.1" // プレースホルダーを追加
          />
        </div>
        <div>
          <label className="block">毎月の積立額 (万円)</label>
          <input
            type="number"
            value={monthlyInvestment}
            onChange={(e) => setMonthlyInvestment(e.target.value)}
            className="w-full p-2 border border-gray-300 rounded"
            placeholder="例: 5" // プレースホルダーを追加
          />
        </div>
        <div>
          <label className="block">選択モデル</label>
          <select
            value={modelKey}
        ...
        <div>
          <label className="block">為替レート (円/USD)</label>
          <input
            type="number"
            value={usdJpyRate}
          ...
          <table className="w-full border-collapse border border-gray-300">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="border px-4 py-2">CAGR</th>
                <th className="border px-4 py-2">1BTC価格</th>
                <th className="border px-4 py-2">年間積立額</th>
                <th className="border px-4 py-2">追加BTC</th>
                <th className="border px-4 py-2">累計BTC</th>
                <th className="border px-4 py-2">資産評価額</th>
              </tr>
            </thead>
            <tbody>
              {results.map((result, idx) => (
                <tr key={idx}>
                  ...
                  <td className="border px-4 py-2">{result.cagr}</td>
                  <td className="border px-4 py-2">{result.btcPrice}</td>
                  <td className="border px-4 py-2">{result.annualInvestment}</td>
                  <td className="border px-4 py-2">{result.btcPurchased}</td>
                  <td className="border px-4 py-2">{result.totalBtc}</td>
                  <td className="border px-4 py-2">{result.totalValue}</td>
                </tr>
              ))}
            </tbody>
          </table>
